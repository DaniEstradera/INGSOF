...
{
	public static Vector3 Shake(float amplitude, float frequency, int octaves, float persistance, float lacunarity, float burstFrequency, int burstContrast, float time, Vector3 offset)
	{
		float valX = 0;
		float valY = 0;
		...
		return new Vector3(offset.x + valX, offset.y + valY, offset.z);
	}
}